...
author: "GRUPO 8. Emilia Coleto y Manuel García"
date: "28/12/2021"
output:
...
##Pasar dataset a csv
#write.csv(heart3,"heart3_ApSuperv.csv")


```

```{r Pasar dataset preprocesado a .csv para el análisis supervisado}
...
```{r Análisis a realizar: SUPERVISADO}
```
##Paquete Caret 

Contiene todo lo necesario para realizar funciones de facilitación, mejora y optimización respecto a la creación de modelos predictivos. Con este paquete realizaremos diversas técnicas computacionales con las que podremos comparar y extraer el mejor modelo de predicción para nuestro problema.
Para ello, el primer paso será instalar y cargar el paquete en nuestro entorno de trabajo.

```{r Instalación y carga de paquete Caret}
...
Lectura del heart3_ApSuperv.csv, el cual es el csv proveniente del dataframe final elaborado en el preprocesamiento. Este csv contiene una columna nueva ("X") generada al usar el write.csv. Mediante dplyr eliminaremos dicha columna y tendremos el nuevo dataset limpio para trabajar
```{r}


asuper <- read.csv("heart3_ApSuperv.csv")
asuper2 <- select(asuper, -X) #eliminamos columna X
asuper2
...
A continuación, mostraremos los diferentes tipos de validaciones cruzadas que aplicaremos para el entrenamiento de nuestro modelo de predicción con la ayuda de la función trainControl(). Además de calcular la probabilidad de la clasificación, calcularemos la probabilidad de las predicciones (Curva ROC).
